...
IP_PRIVADA_FRONT_NFS_CLIENT=172.31.45.202
...
echo "define('WP_SITEURL', 'http://$IP_PUBLICA_BALANCEADOR/wordpress');" >> wp-config.php
echo "define('WP_HOME', 'http://$IP_PUBLICA_BALANCEADOR');" >> wp-config.php
...
# Instalamos paquetes necesarios para servidor NFS
apt install nfs-kernel-server -y

# Cambiamos grupo y propietario al directorio /var/www/html
chown nobody:nogroup /var/www/html

# Configuiramos el archivo /etc/exports
echo "/var/www/html $IP_PRIVADA_FRONT_NFS_CLIENT(rw,sync,no_root_squash,no_subtree_check)" > /etc/exports
